...
for x in range(1,x_train.shape[0]):
    model1=KNeighborsRegressor(n_neighbors=x)
    model1.fit(x_train,y_train)
    score=model1.score(x_test,y_test)
    mean=mean_absolute_error(y_test,model1.predict(x_test))
    score_dict[x]=score
    mean_dict[x]=mean
    if score_list<score:
        score_list=score
    ...
    if mean_list>mean:
        mean_list=mean
        k1_list=x
print(f'최고점수: {score_list}, k_neighbor: {k_list}')
print(f'최고mean: {mean_list}, k_neighbor: {k1_list}')

a=plt.twinx()
ys=np.array(list(mean_dict.values()))
plt.plot(mean_dict.keys(),ys/ys.max(),'bo')
a.plot(list(score_dict.keys()),list(score_dict.values()),'rs')
plt.show()
