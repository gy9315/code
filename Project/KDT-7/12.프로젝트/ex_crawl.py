...
count=0
url_list=[]
for post_id in range(1,12): 
    url=f'https://www.wevity.com/?c=find&s=1&mode=ing&gub=1&sp=name&sw=%EA%B3%B5%EB%AA%A8%EC%A0%84&gp={post_id}'
    driver.get(url)
    time.sleep(2)
    soup=BeautifulSoup(driver.page_source,'html.parser')
    links=soup.select('div.tit > a[href]')
    for x in links:
...
DF.loc[:,'url']=pd.DataFrame([base_url_list]).T
DF.drop(columns=[5],inplace=True)
DF.columns=['name','category','qual','company','total_prize','first_prize','register_start','register_end','url']
DF.to_csv('info1.csv')
