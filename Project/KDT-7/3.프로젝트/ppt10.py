...
sum_value=pd.Series(sum_value).rolling(window=20,min_periods=1).mean()
plt.plot(global_temp['Time'][:-11],sum_value,color='y',label='Sun Ifrared radiation')
# ---------------------------------------------------------------------------------------
# [green house gas emmision]
colors=['#003049','#d62828','#f77f00']
print(gb_mergeDF.columns)
plt.plot(gb_mergeDF['Year'],gb_mergeDF['Co2'],label="Co2's emmision",color='purple')
# --------------------------------------------------------------------------------------
plt.fill([1945,1980,1980,1945],[0.6,0.6,0.3,0.3])
