...
plt.plot(global_temp["Time"][:-11],sum_value,'go--',ms=2,linewidth=1,c='goldenrod',label='Sun Ifrared rays(nm)')
plt.fill_between(global_temp["Time"][:-11],sum_value-0.01,sum_value+0.01,color='r',alpha=0.2)
# 이동평균 추세선 그리기
# sum_value=pd.Series(sum_value).rolling(window=20,min_periods=1).mean()
# plt.plot(global_temp['Time'][:-11],sum_value,color='y')
# --------------------------------------------------------------------------------------
