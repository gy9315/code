...
# # # 2015년
전주DF=grdpDF[(grdpDF['주소']=='전북특별자치도 전주시')]
임실DF=grdpDF[(grdpDF['주소']=='전북특별자치도 임실군')]
장수DF=grdpDF[(grdpDF['주소']=='전북특별자치도 장수군')]
print(전주DF)
army_27DF=pd.concat([전주DF.iloc[:,:].reset_index(drop=True),임실DF.iloc[:,:].reset_index(drop=True),장수DF.iloc[:,:].reset_index(drop=True)])
...
for x in range(army_27DF.shape[0]):
    if x%2==0:    
        ...
        axes[x//2].axvline(x=2006*2+0.4, color='k', linestyle='--', linewidth=2, label='착공연도')
        axes[x//2].axvline(x=2014*2+0.4, color='r', linestyle='--', linewidth=2, label='이전연도')
    else:
