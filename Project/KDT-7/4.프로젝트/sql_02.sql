...
select count(번호) from restaurant_statics;
select * from restaurant_statics;
# 생겨난 음식점 수
# 속성변경(date타입)
update restaurant_statics set 인허가일자=insert(인허가일자,8,3,'');  
update restaurant_statics set 폐업일자=insert(폐업일자,8,3,'');  
update restaurant_statics set 인허가일자=concat(인허가일자,'-01');
update restaurant_statics set 폐업일자=concat(폐업일자,'-01');
update restaurant_statics set 폐업일자=replace(폐업일자,'-01','') where 폐업일자='-01';
update restaurant_statics set 폐업일자=cast(폐업일자 as date) where 폐업일자!='';
update restaurant_statics set 인허가일자=replace(인허가일자,'-01','1995-01-01') where 인허가일자='-01';
update restaurant_statics set 인허가일자=cast(인허가일자 as date) where 인허가일자!='';
select count(*) from restaurant_statics;
# 주소지 통일시키기
# [1] 최빈값 찾기
drop view if exists most_common_address;
CREATE VIEW most_common_address AS
SELECT 개방자치단체코드, 소재지전체주소,
       COUNT(*) AS cnt,
       ROW_NUMBER() OVER (PARTITION BY 개방자치단체코드 ORDER BY COUNT(*) DESC) AS rn
FROM restaurant_statics
GROUP BY 개방자치단체코드, 소재지전체주소;
# [2] 최빈값 선택
drop view if exists most_common_address_filtered;
CREATE VIEW most_common_address_filtered AS
SELECT 개방자치단체코드, 소재지전체주소
FROM most_common_address
WHERE (개방자치단체코드, length(소재지전체주소)) IN (
    SELECT 개방자치단체코드, MAX(length(소재지전체주소))
    FROM most_common_address
    GROUP BY 개방자치단체코드
);
SELECT * FROM most_common_address_filtered WHERE 개방자치단체코드 = 3020000;
SELECT 개방자치단체코드, 소재지전체주소 FROM restaurant_statics WHERE 개방자치단체코드 = 3020000 and 소재지전체주소 like '경기%';
SELECT 개방자치단체코드, COUNT(DISTINCT SUBSTRING_INDEX(소재지전체주소, ' ', 1)) AS 서로다른주소개수
FROM restaurant_statics
GROUP BY 개방자치단체코드
HAVING 서로다른주소개수 > 1;
...
# [3] update
UPDATE restaurant_statics a
JOIN most_common_address_filtered b
ON a.개방자치단체코드 = b.개방자치단체코드
SET a.소재지전체주소 = b.소재지전체주소
where a.소재지전체주소='';
select count(*) from restaurant_statics where 소재지전체주소='';
# -------------------------------------------------------------
# 소재지 비워져 있는 주소칸 채우기
# 1번 innerjoin으로 테이블 만든다
# 2번 null 값에 주소값 집어넣는다
-- update 
-- restaurant_statics a join (select 개방자치단체코드 , max(소재지전체주소) as 소재지전체주소 from restaurant_statics where 소재지전체주소!='' group by 개방자치단체코드) as b
-- on a.개방자치단체코드 = b.개방자치단체코드
-- set a.소재지전체주소=b.소재지전체주소
-- where a.소재지전체주소='';
-- 
-- select * from restaurant_statics where 소재지전체주소='';
# 파이썬 작업 ->
# 새로 생긴 음식점 view만들기
...
from restaurant_statics
group by 개방자치단체코드, year(인허가일자),주소
order by 개방자치단체코드,year(인허가일자);
select * from open_food where 주소 ='';
...
on o1.연도=c1.연도 and o1.개방자치단체코드=c1.개방자치단체코드 and o1.주소=c1.주소
