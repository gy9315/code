...
# ============================================================================
s=MinMaxScaler()
# xlabel=s.fit_transform(featurDF[['대구 달서구']][132:])
xlabel=(featurDF[['대구 달서구']][132:].values)
x_train,x_test,y_train,y_test=train_test_split(xlabel,target_scale(targetDF.T['대구 달서구'][:-2].values))

model=make_pipeline(PolynomialFeatures(degree=1),KNeighborsRegressor(n_neighbors=14))
...
for x in ['대전 유성구','부산 남구','광주 남구','대구 달서구']:
    ...
    print(f'score: {score}, RMSE: {root_mean_squared_error(target_scale(targetDF.T[x][:-2].values),pred)}, RMSE: {mean_absolute_error(target_scale(targetDF.T[x][:-2].values),pred)}')