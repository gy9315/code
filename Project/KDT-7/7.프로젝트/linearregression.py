...
# ============================================================================
s=StandardScaler()
# xlabel=s.fit_transform(featurDF[['서울 /양천구']][132:])
xlabel=(featurDF[['서울 양천구']][132:].values)
x_train,x_test,y_train,y_test=train_test_split(xlabel,(target_scale(targetDF.T['서울 양천구'][:-2].values)))

model=make_pipeline(PolynomialFeatures(degree=2),RidgeCV(alphas=np.arange(0.1,1000,0.1)))
...
print(f'score: {score}, RMSE: {root_mean_squared_error(y_train,pred)}, MAE: {mean_absolute_error(y_train,pred)}')
print(f'score: {score1}, RMSE: {root_mean_squared_error(y_test,pred1)}, MAE: {mean_absolute_error(y_test,pred1)}')
# ============ 선형회귀 제한===========================
for x in ['대전 유성구','부산 남구','광주 남구','대구 달서구']:
    pred=model.predict(featurDF[x][132:].values)
    score=model.score(featurDF[x][132:].values,target_scale(targetDF.T[x][:-2].values))
    # print(pred)
    # print(featurDF[x][132:])
    print(f'score: {score}, RMSE: {root_mean_squared_error(target_scale(targetDF.T[x][:-2].values),pred)}, MAE: {mean_absolute_error(target_scale(targetDF.T[x][:-2].values),pred)}')