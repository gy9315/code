...
class LSTModel(nn.Module):
    def __init__(self,word_vector):
        ...
        global DEVICE
        self.embeding=nn.Embedding.from_pretrained(word_vector,freeze=False,padding_idx=0)
        self.LSTM=nn.LSTM(128,128,3,batch_first=True,dropout=0.3,device=DEVICE,bidirectional=True)
    ...
    def forward(self,data):
        ...
        hd=hd[-1].permute(1,0,2)
        hd=hd.countiguous().view(hd.shape[0],-1)
...
def collate_batch_offset(batch,vocab):
    ...
    for label, target in batch:
        label_list.extend([vocab.get(x,vocab['<ukn>']) for x in label.item()])
...
def collate_batch(batch,vocab):
    ...
    for label,target in batch:
        label_list.append(torch.LongTensor([vocab.get(x,vocab['<ukn>']) for x in label.item()]))
...
# ==========================================================================================================================
# function
def normal_train(model_class,word_vecotor,feature,target,collate_fn,weight):
    ...
    accur=Accuracy(task='multiclass',num_classes=8)
    for epoch in range(EPOCH):
        ...
        if LR.num_bad_epochs >= LR.patience:
            print('Early Stopping')
            torch.save(model_class.state_dict(),f'{model_class}.pkl') 
            break
