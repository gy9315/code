import pandas as pd
import numpy as np
from scipy import stats,integrate
import matplotlib.pyplot as plt
import koreanize_matplotlib
from collections import Counter
# =====================================================================
# 저번주 데이터터
example=pd.read_csv('./DATA/lastweek.csv',encoding='euc-kr')
example=example.drop(columns=['지점','지점명'])
example['일시']=pd.to_datetime(example['일시'],format=('%Y-%m-%d'))
# print(example.columns)
example.columns=['일시', '평균기온', '일강수량', '평균풍속', '평균기압','평균전운량']
example['일강수량']=example['일강수량'].fillna(0)
example1=example['일시'].dt.quarter
example=pd.concat([example,example1],axis=1)
example.columns=['일시', '평균기온', '일강수량', '평균풍속', '평균기압', '평균전운량','분기']
example=example.sort_values(by='일시',axis=0)
year_list=example['일시'].dt.year.unique()
quater_list=range(1,5)
total=[]
for x in year_list:
    year_example=example[example['일시'].dt.year==x]
    for y in quater_list:
        quater_example=year_example[year_example['분기']==y]
        week_quaterexample=quater_example.groupby(quater_example['일시'].dt.isocalendar()['week'])
        for a in week_quaterexample:
            total.append(a[1].mean().values)
example=pd.DataFrame(total)
example.columns=['일시', '평균기온', '일강수량', '평균풍속', '평균기압', '평균전운량','분기']
example.loc[:,'평균기온':]=example.loc[:,'평균기온':].apply(lambda x:round(x,2))

# =====================================================================
DF=pd.read_csv('./DATA/temperature_total.csv',encoding='utf-8',index_col=0)
# print(DF)
DF=DF.drop(columns=['지점','지점명'])
# print(DF)
# print(DF.dtypes)
DF['일시']=pd.to_datetime(DF['일시'],format=('%Y-%m-%d'))
# print(DF.dtypes)
# print(DF.columns)
DF.columns=['일시', '평균기온', '일강수량', '평균풍속', '평균기압','평균전운량']
# # print(DF)
DF['일강수량']=DF['일강수량'].fillna(0)
DF1=DF['일시'].dt.quarter
# # print(DF1.index)
DF=pd.concat([DF,DF1],axis=1)
DF.columns=['일시', '평균기온', '일강수량', '평균풍속', '평균기압', '평균전운량','분기']
# print(DF)
DF=DF.sort_values(by='일시',axis=0)
year_list=DF['일시'].dt.year.unique()
# print(year_list)
quater_list=range(1,5)
# 각 연도에 해당 분기에 주차별로 평균기온: 평균전운량 평균내기
total=[]
for x in year_list:
    year_DF=DF[DF['일시'].dt.year==x]
    for y in quater_list:
        quater_DF=year_DF[year_DF['분기']==y]
        week_quaterDF=quater_DF.groupby(quater_DF['일시'].dt.isocalendar()['week'])
        for a in week_quaterDF:
            total.append(a[1].mean().values)
DF=pd.DataFrame(total)
DF.columns=['일시', '평균기온', '일강수량', '평균풍속', '평균기압', '평균전운량','분기']
# print(DF)
# # 평균기온 밑 2자리, 강수량, 밑 2자리, 평균풍속 밑 2자리, 평균기강, 밑 2자리, 전운량 밑2자리
DF.loc[:,'평균기온':]=DF.loc[:,'평균기온':].apply(lambda x:round(x,2))
# # 2020년 기준 특정 index 값일 일치한다면, 그다음주 가 일치할 확률 구하기


# np.random.seed(100)
check_DATA_statics=[]
print(example)
for x in range(1):
    example=example.loc[0]
    # example1=DF.loc[x+1]
    match_list=[]
    next_list=[]    
    # print(example1)
    for x in year_list[1:]:
        year_DF=DF[DF['일시'].dt.year==x]
        # 평균기온 오차 정하기
        if example['평균기온']<0:
            a=year_DF['평균기온'].between(example['평균기온']*1.2,example['평균기온']*0.8)
        else:
            a=year_DF['평균기온'].between(example['평균기온']*0.8,example['평균기온']*1.2)
        # print(example['평균기온']*0.9)
        b=year_DF['평균풍속'].between(example['평균풍속']*0.8,example['평균풍속']*1.2)
        c=year_DF['평균기압'].between(example['평균기압']*0.996,example['평균기압']*1.004)
        d=year_DF['평균전운량'].between(example['평균전운량']*0.9,example['평균전운량']*1.1)
        if example['분기'] in [1,4]:
            valueDF=year_DF[a & b & c & d & ((year_DF['분기']==1) |(year_DF['분기']==4))]
        else: valueDF=year_DF[a & b & c & d & ((year_DF['분기']==2) |(year_DF['분기']==3))]
        value=valueDF.values
        # print(valueDF)
        try:
            for y in range(len(value)):
                if len(value)!=0:   
                    match_list.append(value[y])     
                    # print(value)       
                else: pass
            if len(value)!=0:
                for idx in valueDF.index:
                    next_list.append(valueDF.loc[idx].values)
            else: pass
        except: pass   

    # print('='*40)
    match_DF=pd.DataFrame(match_list)
    try:
        match_DF.columns=['일시', '평균기온', '일강수량', '평균풍속', '평균기압', '평균전운량','분기']
    except: pass
    # print(match_DF['일강수량'].mean())
    next_DF=pd.DataFrame(next_list)
    try:
        next_DF.columns=['일시', '평균기온', '일강수량', '평균풍속', '평균기압', '평균전운량','분기']
    except: pass
    # print(next_list)
    # print(next_DF)
    try:
        bin=np.arange(0,next_DF['일강수량'].max()+0.9,0.5)
        label=['강수없음']
        label.extend(bin[1:])
        b=pd.cut(next_DF['일강수량'],bins=bin,labels=bin[:-1],right=False)
    # 강수 0 ~ 0.5 0.5 ~ 1.0, 미만 강수가 없음
    # =================================================================
    # example1데이터의 구간데이터로 변환
        # check_DATA=example1['일강수량']
        # check_DATA=round(check_DATA*2)/2
        stDF=pd.DataFrame(b)
        stDF=pd.concat([next_DF['일시'].dt.year,stDF],axis=1)
    except:pass
    # =================================================================
    # print(stDF)
    # 연도별 거리별 차별화
    # 단계 1970 ~ 1977 
    # +1
    # 단계 1978 ~ 1995
    # +2
    # 단계 1996 ~ 2014
    # +3
    # 단계 2015 ~ 2021
    # +4
    # 단계 2022 ~ 
    # =============================
    binlist=[]
    distance={1**4:(1970,1977),2**4:(1978,1995),3**4:(1996,2014),4**4:(2015,2021),5**4:(2022,2025)}
    try:
        for x in distance:
            for y in range(stDF.shape[0]):
                if stDF.loc[y,'일시'] in range(distance.get(x)[0],distance.get(x)[1]):
                    bin_value=[stDF.loc[y,'일강수량']]*x
                    binlist.extend(bin_value)
        bin_prob={x:y/len(binlist) for x,y in Counter(binlist).items()}
        f=np.polyfit(list(bin_prob.keys()),list(bin_prob.values()),3)
        poly_ld=np.poly1d(f)
        print(bin_prob)
        xs=np.linspace(min(bin_prob.keys()),max(bin_prob.keys()),100)

        # print(check_DATA)
        # a=poly_ld(check_DATA)
        # if a>=1:
        #     a=1
        #     check_DATA_statics.append(a)
        # elif a<=0:
        #     a=0
        #     check_DATA_statics.append(a)
        # else:
        #     check_DATA_statics.append(a)
        # print(check_DATA)
        # print(a)
        plt.plot(xs,poly_ld(xs))
        plt.show()
        plt.plot
    except: pass
binDF=pd.DataFrame(bin_prob,index=['stats'])
binDF=binDF.apply(lambda x:round(x,3)*100 )
binDF=binDF.sort_index(axis=1)
print(binDF)
# plt.figure()
# # print(sum(check_DATA_statics)/len(check_DATA_statics))
# new=[]
# # bin=np.arange(0,max(check_DATA_statics)+0.1,0.1)
# # print(bin)
# new.extend(bin[1:])
# # print(label)
# # print(bin)
# # print(next_DF['일강수량'])
# b=pd.cut(check_DATA_statics,bins=bin,labels=bin[:-1],right=False)
# # print(b)
# print(check_DATA_statics)
# plt.hist(b,bins=len(set(b)),range=(min(b),max(b)),density=True)
# plt.show()

# =====================================================================
# 데이터 검증
# 2022년 부터 한 주씩 들고오기
# 다음주에 데이터가 강수량을 집어넣고 확률 추출
# 전체 데이터의 확률을 평균 내기
